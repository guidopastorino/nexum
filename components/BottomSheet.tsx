...
const BottomSheet = ({ trigger, children, isOpen, setOpen }: BottomSheetProps) => {
  ...
  return (
    <Drawer.Root open={openState} onOpenChange={handleOpenChange}>
      ...
      <Drawer.Portal>
        <Drawer.Overlay className="drawerOverlay fixed inset-0 bg-black/60 z-50" onClick={() => toggleOpen(false)} />
        <Drawer.Content className="dark:bg-neutral-900 bg-white text-black dark:text-white h-fit fixed bottom-0 left-0 right-0 outline-none z-50 pt-1 rounded-t-lg">
